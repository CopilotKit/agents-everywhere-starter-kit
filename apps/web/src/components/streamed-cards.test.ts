...
test("incident card preserves the headline while other fields are streaming", () => {
  const html = renderToStaticMarkup(
    createElement(IncidentCard, { headline: "Checkout unavailable" }),
  );
...
test("incident card tolerates partial arrays and nested entries", () => {
  const html = renderToStaticMarkup(
    createElement(IncidentCard, {
      tone: "att",
      facts: [
        null,
        {},
        { label: "Impact" },
        { label: "Since", value: "10:00" },
      ],
      nextSteps: [null, "Check deployment"],
    }),
  );
  assert.match(html, /var\(--ae-muted\)/);
...
test("timeline renders loading content for empty and title-only arguments", () => {
  assert.match(
    renderToStaticMarkup(createElement(Timeline, {})),
    /Preparing timeline/,
  );
  const html = renderToStaticMarkup(
    createElement(Timeline, { title: "Incident history" }),
  );
  assert.match(html, /Incident history/);
  assert.match(html, /Preparing timeline/);
  assert.match(
    renderToStaticMarkup(createElement(Timeline, { columns: ["Time"] })),
    /Loading events/,
  );
  assert.match(
    renderToStaticMarkup(createElement(Timeline, { rows: [["10:00"]] })),
    /Preparing timeline/,
  );
  assert.match(
    renderToStaticMarkup(
      createElement(Timeline, { columns: null, rows: null }),
    ),
    /Preparing timeline/,
  );
});
...
test("complete incident and timeline arguments render their content", () => {
  const card = renderToStaticMarkup(
    createElement(IncidentCard, {
      ...
      summary: "All customers can check out",
      facts: [{ label: "Errors", value: "0%" }],
      nextSteps: ["Monitor"],
      tone: "good",
    }),
  );
